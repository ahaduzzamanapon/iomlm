<x-teacher-layout>
    ...
    <div class="page-header">
        <div class="page-header-left">
            <h1>পুনরায় পরীক্ষার আবেদনসমূহ (Re-Exam Appeals)</h1>
            <p>আপনার বিষয়ের পরীক্ষাগুলোতে শিক্ষার্থীদের পুনরায় পরীক্ষার আবেদন পর্যালোচনা ও অনুমোদন</p>
        </div>
    </div>
    ...
    <div style="display:flex;gap:10px;margin-bottom:20px;flex-wrap:wrap">
        <a href="{{ route('teacher.exams.appeals.index', ['status' => 'PENDING']) }}" class="btn btn-sm {{ $status === 'PENDING' ? 'btn-primary' : 'btn-outline' }}">
            <i class="fa-regular fa-clock"></i> অপেক্ষমাণ (Pending)
        </a>
        <a href="{{ route('teacher.exams.appeals.index', ['status' => 'APPROVED']) }}" class="btn btn-sm {{ $status === 'APPROVED' ? 'btn-primary' : 'btn-outline' }}">
            <i class="fa-solid fa-check"></i> অনুমোদিত (Approved)
        </a>
        <a href="{{ route('teacher.exams.appeals.index', ['status' => 'REJECTED']) }}" class="btn btn-sm {{ $status === 'REJECTED' ? 'btn-primary' : 'btn-outline' }}">
            <i class="fa-solid fa-xmark"></i> বাতিল (Rejected)
        </a>
        <a href="{{ route('teacher.exams.appeals.index', ['status' => 'ALL']) }}" class="btn btn-sm {{ $status === 'ALL' ? 'btn-primary' : 'btn-outline' }}">
            সকল আবেদন (All)
        </a>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @elseif($appeal->isApproved())
                                ...
                                <div style="font-size:10px;color:#047857;margin-top:2px">অনুমোদন: {{ $appeal->reviewer?->name ?? 'Teacher' }}</div>
                            @else
                        ...
                        <td style="text-align:right;white-space:nowrap">
                            @if($appeal->isPending())
                                <form method="POST" action="{{ route('teacher.exams.appeals.approve', $appeal) }}" style="display:inline" onsubmit="return confirm('এই আপিলটি অনুমোদন করতে চান? শিক্ষার্থীর পূর্বের খাতা রিসেট হয়ে যাবে এবং নতুন করে পরীক্ষা দিতে পারবে।')">
                                    @csrf
                                ...
                                <form method="POST" action="{{ route('teacher.exams.appeals.reject', $appeal) }}" style="display:inline" onsubmit="return confirm('এই আপিলটি বাতিল করতে চান?')">
                                    @csrf
