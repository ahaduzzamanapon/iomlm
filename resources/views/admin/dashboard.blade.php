<x-admin-layout>
    ...
    <div class="page-header">
        <div class="page-header-left">
            ...
            <p>System overview — {{ now()->format('l, d M Y') }}</p>
        </div>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            <div class="stat-icon green">
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
            </div>
            <div class="stat-info">
        ...
        <div class="stat-card">
            <div class="stat-icon red">
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
            </div>
            <div class="stat-info">
    ...
    <div class="grid-2">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="td-muted">{{ $admission->created_at->format('d M') }}</td>
                            <td>
                                <a href="{{ route('admin.admissions.show', $admission) }}" class="btn btn-outline btn-sm">Review</a>
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="4" style="text-align:center;padding:20px;color:var(--text-muted)">No pending admissions</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="card">
            ...
            <div style="padding:0">
                ...
                <div style="display:flex;align-items:center;gap:12px;padding:12px 20px;border-bottom:1px solid var(--card-border)">
                    <div style="width:38px;height:38px;background:linear-gradient(135deg,#3b82f6,#8b5cf6);border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:700;flex-shrink:0">
                        {{ strtoupper(substr($batch->name,0,2)) }}
                    </div>
                    <div style="flex:1">
                        <div style="font-size:13px;font-weight:600">{{ $batch->name }}</div>
                        <div style="font-size:11px;color:var(--text-muted)">{{ $batch->course->name ?? '—' }}</div>
                    </div>
                    <span class="badge badge-active">Active</span>
                </div>
                @empty
                <div class="empty-state"><p>No active batches</p></div>
                @endforelse
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div style="padding:16px;display:grid;grid-template-columns:1fr 1fr;gap:10px">
                <a href="{{ route('admin.subjects.index') }}" class="btn btn-outline" style="justify-content:flex-start;padding:12px;font-size:13px">
                    📖 Subjects & Modules
                </a>
                <a href="{{ route('admin.courses.index') }}" class="btn btn-outline" style="justify-content:flex-start;padding:12px;font-size:13px">
                    🎓 Courses & Semesters
                </a>
                <a href="{{ route('admin.teachers.index') }}" class="btn btn-outline" style="justify-content:flex-start;padding:12px;font-size:13px">
                    👨‍🏫 Teachers & Assignments
                </a>
                <a href="{{ route('admin.batches.index') }}" class="btn btn-outline" style="justify-content:flex-start;padding:12px;font-size:13px">
                    👥 Batches & Timelines
                </a>
            </div>
        </div>
